...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source_ids=None, source_mask=None, target_ids=None, target_mask=None, args=None):
        ...
        if target_ids is not None:  # For training
            ...
            # Shift to make the output sequence align with target sequence
            ...
            for i in range(source_ids.shape[0]):
                ...
                input_ids = torch.zeros((1, 1)).fill_(self.sos_id).long().to(
                    source_ids.device)  # SOS token initialization
                ...
                for _ in range(self.max_length):
                    ...
                    next_token = logits.argmax(dim=-1)  # Get the index of the max logit
                    input_ids = torch.cat([input_ids, next_token.unsqueeze(1)], dim=-1)

                    if next_token.item() == self.eos_id:  # EOS token encountered, stop decoding
                        break
